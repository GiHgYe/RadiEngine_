...
Font* Font::LoadFromFile(std::string font_file_path, unsigned short font_size)
{
    ...
    //读取 ttf 字体文件
    std::ifstream input_file_stream(font_file_path, std::ios::in | std::ios::binary);
    ...
    if (pixels) memset(pixels, 0, font->font_texture_size_ * font->font_texture_size_);
...
void Font::LoadCharacter(char ch)
{
    ...
    Character* character = new Character(
        font_texture_fill_x * 1.0f / font_texture_size_,
